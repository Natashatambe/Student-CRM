...
function DashboardCard({ title, value, icon: Icon, trend = "+12% this month", variant = "cream" }) {
  const getCardStyle = () => {
    switch (variant) {
      ...
      case "coral":
        return "bg-[#cc785c] text-white border-[#cc785c]";
      case "soft":
        return "bg-[#f5f0e8] text-[#141413] border-[#e6dfd8]";
      case "cream":
      default:
  ...
  return (
    <Card className={`overflow-hidden relative transition-all duration-300 border ${getCardStyle()}`}>
      <div className="p-6">
        <div className="flex items-center justify-between">
          <p className="text-xs font-medium uppercase tracking-wider opacity-80">{title}</p>
          {Icon && (
            <div className={`h-9 w-9 rounded-md flex items-center justify-center ${variant === 'dark' ? 'bg-[#252320] text-[#cc785c]' : variant === 'coral' ? 'bg-white/20 text-white' : 'bg-[#faf9f5] text-[#cc785c] border border-[#e6dfd8]'}`}>
        ...
        <div className="mt-4 flex items-baseline justify-between">
          <h2 className="text-3xl font-normal tracking-tight font-serif-display">{value}</h2>
        </div>
