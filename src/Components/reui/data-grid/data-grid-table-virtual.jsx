...
function getDataGridTableHeaderOffset({
  ...
  const headerElement = containerElement.querySelector(':scope > [data-slot="data-grid-table"] > thead')
...
function DataGridTableVirtualBody(
  ...
  topRows.forEach((row, index) => {
    renderedRows.push(<DataGridTableRenderedRow
      key={row.id}
      row={row}
      pinnedBoundary={
  ...
  if (isVirtualizationEnabled) {
    if (leadingSpacerHeight > 0) {
      renderedRows.push(
        <DataGridTableVirtualSpacer key="virtual-spacer-start" table={table} height={leadingSpacerHeight} />
      )
    }
    ...
    virtualItems.forEach((virtualRow) => {
      ...
      renderedRows.push(
        <DataGridTableRenderedRow key={row.id} row={row} rowRef={measureRowRef} rowIndex={virtualRow.index} />
      )
    })

    if (trailingSpacerHeight > 0) {
      renderedRows.push(
        <DataGridTableVirtualSpacer key="virtual-spacer-end" table={table} height={trailingSpacerHeight} />
      )
    }
  } else {
  ...
  if (showFetchingRow) {
    renderedRows.push(<DataGridTableVirtualStatusRow key="virtual-status-loading" table={table}>
      <div className="flex items-center justify-center gap-2">
        <Spinner className="size-4 opacity-60" />
        {loadingMoreMessage}
      </div>
  ...
  if (showCompleteRow) {
    renderedRows.push(
      <DataGridTableVirtualStatusRow key="virtual-status-complete" table={table} className="py-3 text-xs">
        {allRowsLoadedMessage}
      </DataGridTableVirtualStatusRow>
    )
  }
