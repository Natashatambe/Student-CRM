...
const headerCellSpacingVariants = ({
  size
}) =>
  size === "dense" ? "px-2 h-8" : "px-3"

const bodyCellSpacingVariants = ({
  size
}) =>
  size === "dense" ? "px-2 py-1.5" : "px-3 py-2"

const footerCellSpacingVariants = ({
  size
}) =>
  size === "dense" ? "px-2 py-1.5" : "px-3 py-2"
...
// built-in toggle.
function getDataGridTreeIndentStyle(row, indent = 20) {
  return {
 ...
 */
function getDataGridScrollAreaViewport(node) {
  const scrollViewport = node.closest('[data-slot="scroll-area-viewport"]')
...
function findTouchClientX(list, identifier) {
...
function startDataGridColumnResizeOnEnd(event, header, table) {
  ...
  const startOffset =
    headerRect &&
    Number.isFinite(table.options.columnResizeDirection === "rtl"
      ? headerRect.left
      : headerRect.right)
  ...
  const columnSizingStart = header
    .getLeafHeaders()
    .map((leafHeader) =>
    [leafHeader.column.id, leafHeader.column.getSize()])
  ...
  const updateOffset = (clientXPos, commit = false) => {
    ...
    const deltaPercentage = Math.min(Math.max(
...
function getDataGridTableOrderedVisibleColumns(table) {
  return [
...
function getDataGridTableOrderedVisibleCells(row) {
  return [
...
function getDataGridTableMergedHeaderGroups(table) {
...
function hasDataGridTableRightPinnedColumns(table) {
...
function DataGridTableViewport({
  ...
  const stopContainerObserverRef = useRef(null)
 ...
 */
const DATA_GRID_DEFAULT_COLUMN_RESIZE_MODE = "onEnd"
...
function DataGridTableHeadRowCellResize(
  ...
  const isResizeModeOnEnd =
    getDataGridColumnResizeMode(props.tableLayout?.columnsResizeMode, table.options.columnResizeMode) === "onEnd"
  const stopResizeSessionRef = useRef(undefined)
      ...
      {...{
        ...
        className: cn(
          ...
          isLastVisibleColumn
            ...
            : isPinned
              ? cn(// A pinned column is sticky, so the handle sits inside the
          // cell instead of straddling the boundary, where the next
          // sticky cell would paint over it.
          "end-0 w-5 justify-end", // With the pin affordance on, the pinned edge already draws
...
function DataGridTableResizeIndicator({
  ...
  // on every mousemove.
  const headerHeightCacheRef = useRef({ key: false, value: 0 })
  const columnResizing = table.state.columnResizing
  const resizingColumnId = columnResizing.isResizingColumn
  const resizeMode = getDataGridColumnResizeMode(props.tableLayout?.columnsResizeMode, table.options.columnResizeMode)
  const isActive = !!(
    props.tableLayout?.columnsResizable &&
    resizeMode === "onEnd" &&
    resizingColumnId
...
function DataGridTableRowSpacer() {
  return (
    <tbody
      aria-hidden="true"
      className="h-2"
      data-slot="data-grid-table-body-spacer"></tbody>
  );
}
...
function DataGridTableBody({
  ...
  return (
    <tbody
      ...
      className={cn(props.tableLayout?.rowRounded &&
        "[&_td:first-child]:rounded-l-lg", props.tableLayout?.rowRounded &&
        "[&_td:last-child]:rounded-r-lg", props.tableClassNames?.body)}>
      {children}
    </tbody>
  );
}
  ...
  return (
    <Fragment>
      <DataGridTableBodyRow
        ...
        rowRef={rowRef}
        dataIndex={rowIndex}>
        {[...startVisibleCells, ...centerVisibleCells].map((cell) => (
...
function DataGridTableRowExpand(
  {
    row,
    indent = 20,
    className,
...
function DataGridTableBodyRows(
  {
    table
  }
) {
